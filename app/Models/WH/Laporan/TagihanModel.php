...
class TagihanModel extends Model
{
    ...
    public function getDetailTagihanMutasiToko($idGudang, $idToko, $statusPelunasan, $tanggalAwal, $tanggalAkhir, $kataKunciPencarian, $tampilTagihanBelumLunas, $compileOnly = false)
    {	
        $baseQuery  =   $this->db->table("t_tokonotamutasipembayaran A", true);
        $baseQuery->select("C.NAMA AS NAMATOKO, B.NOTAMUTASINOMOR, A.NOTAMUTASIPEMBAYARANNOMOR, A.STATUS, DATE_FORMAT(A.JATUHTEMPO, '%d %b %Y') AS TANGGALJATUHTEMPO, A.PEMBAYARANKE,
                        A.KETERANGAN, A.NOMINAL, IF(A.BUKTIBAYAR IS NULL OR A.BUKTIBAYAR = '', '', CONCAT('".URL_BUKTI_PEMBAYARAN."', A.BUKTIBAYAR)) AS BUKTIBAYAR");
        $baseQuery->join('t_tokonotamutasirekap B', 'A.IDTOKONOTAMUTASIREKAP = B.IDTOKONOTAMUTASIREKAP', 'LEFT');
        $baseQuery->join('m_toko C', 'B.IDTOKO = C.IDTOKO', 'LEFT');
        $baseQuery->where('B.IDGUDANG', $idGudang);
        ...
        $baseQuery->orderBy('C.NAMA, B.NOTAMUTASINOMOR, A.JATUHTEMPO ASC');
