...
use App\Models\POS\Stok\KartuStokModel;
...
class KartuStok extends ResourceController
{
    ...
    public function getDetailKartuStok()
    {
        $rules  =   [
        ...
        $baseData       =   $kartuStokModel->getDetailKartuStok($this->idToko, $idBarangSKU, $tanggalAwal, $tanggalAkhir);
        ...
        if($totalNumberData > 0){
            ...
            foreach($dataDetailKartuStok as $index => $row) {
                if($index == 0) {
                    ...
                    $saldoStok              =   $kartuStokModel->getSaldoStokByTanggalWaktu($this->idToko, $idBarangSKU, $tanggalWaktuStokAwal);
                    $dataSaldoAwal          =   [
                        'NOMORNOTA'             =>  '',
