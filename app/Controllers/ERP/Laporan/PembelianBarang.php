...
use App\Models\ERP\Laporan\PembelianBarangModel;
...
class PembelianBarang extends ResourceController
{
    ...
    {
        $rules  =   [
            'idProdusenDistributor' =>  ['label' => 'Produsen Distributor', 'rules' => 'permit_empty|alpha_numeric'],
        ...
        $messages   =   [
            'idProdusenDistributor' =>  [
                'alpha_numeric' =>  'Produsen distributor yang anda pilih tidak valid, silakan periksa kembali'
            ],
            'tanggalAwal' => [
        ...
        $baseData               =   $pembelianBarangModel->getDataPembelianBarang($idProdusenDistributor, $tanggalAwal, $tanggalAkhir, $kataKunciPencarian);
        ...
        if($totalNumberData > 0){
            ...
            foreach($dataPembelianBarang as $keyPembelianBarang){
                $idNotaPembelianRekap   =   isset($keyPembelianBarang->IDNOTAPEMBELIANREKAP) && $keyPembelianBarang->IDNOTAPEMBELIANREKAP != "" ? $keyPembelianBarang->IDNOTAPEMBELIANREKAP : 0;
                $dataBarangSKU          =   $pembelianBarangModel->getDataBarangSKUPembelian($idNotaPembelianRekap);

                $keyPembelianBarang->DAFTARBARANGSKU    =   $dataBarangSKU;
                unset($keyPembelianBarang->IDNOTAPEMBELIANREKAP);
            }
            ...
            $urlExcelPembelianBarang=   base_url(URL_EXCEL_ERP_DATA_PEMBELIAN_BARANG).$arrParametersEncode;
    ...
    public function excelDataPembelianBarang($arrParametersEncode = null)
    {
        ...
        try {
            ...
            $idProdusenDistributor  =   $arrParameters['idProdusenDistributor'];
            ...
            $arrTitleData   =   ['Laporan Detail Pembelian Barang'];
            $arrFilterData  =   [
                ['Produsen/Distributor', $mainOperation->getDetailProdusenDistributor($idProdusenDistributor)['NAMA']],
            ...
            $arrHeaderData  =   [
                [['A'], 2, 'Nomor Nota'.PHP_EOL.'Produsen/Distributor', 22, 'center'],
                ...
                [['I'], 1, 'Harga Total', 14, 'right', 1],
                [['J'], 2, 'Grand Total', 14, 'right']
            ];
            ...
            if(isset($dataLaporan) && !empty($dataLaporan)){
                foreach($dataLaporan as $keyDataLaporan){
                    $activeWorksheet
                    ->setCellValue('A'.$rowNumberTableContent, $keyDataLaporan->NOTAPEMBELIANNOMOR.PHP_EOL.$keyDataLaporan->NAMAPRODUSENDISTRIBUTOR)
                    ->setCellValue('B'.$rowNumberTableContent, $keyDataLaporan->INPUTTANGGALWAKTU.PHP_EOL.$keyDataLaporan->INPUTUSER)
                    ->setCellValue('C'.$rowNumberTableContent, $keyDataLaporan->KETERANGAN);

                    $idNotaPembelianRekap   =   isset($keyDataLaporan->IDNOTAPEMBELIANREKAP) && $keyDataLaporan->IDNOTAPEMBELIANREKAP != "" ? $keyDataLaporan->IDNOTAPEMBELIANREKAP : 0;
                    $dataBarangSKU          =   $pembelianBarangModel->getDataBarangSKUPembelian($idNotaPembelianRekap);
                    ...
                    if(!empty($dataBarangSKU) && !is_null($dataBarangSKU)){
                        ...
                        foreach($dataBarangSKU as $keyDataBarangSKU){
                            ...
                            ->setCellValue('H'.$rowNumberTableContent, intval($keyDataBarangSKU->HARGABELI))
                            ->setCellValue('I'.$rowNumberTableContent, intval($keyDataBarangSKU->TOTALHARGABELI));

                            $grandTotal     +=  intval($keyDataBarangSKU->TOTALHARGABELI);
